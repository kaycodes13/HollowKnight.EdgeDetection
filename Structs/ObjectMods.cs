...
/// <summary>
/// For specifying which GameObjects (by name) should receive one or more of a
/// set of standard modifications intended to make edge detection look better,
...
record struct ObjectMods(
	...
	public static ObjectMods Union(ObjectMods one, ObjectMods two) {
		...
		static Dictionary<K,V> MergeDict<K,V>(Dictionary<K,V>? a, Dictionary<K,V>? b) {
			Dictionary<K,V> res = [];
			if (a != null) {
				foreach (var entry in a) res.TryAdd(entry.Key, entry.Value);
			}
			if (b != null) {
				foreach (var entry in b) res.TryAdd(entry.Key, entry.Value);
			}
			return res;
		}
	}
